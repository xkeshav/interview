...
export const BlockList = () => {
  ...
  return (
    <div>
      ...
      <p>
        Initially Block colors are green and when user click on a block it will change to red and when all block color changed then reset
        the block color to green in the same order it has been clicked
      </p>
      <p>Click Order: {order}</p>
      {data.map((b, idx) => (
        <Block key={idx} block={b} handleClick={handleFromParent} />
      ))}
    </div>
  );
};
